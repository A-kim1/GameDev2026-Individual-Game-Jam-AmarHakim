...
# Ambil node penting pas scene siap
...
func _physics_process(delta):
	...
	# Gravity (jangan dipaksa 0 biar jatuhnya itu natural gituu)
	if not is_on_floor():
...
func move_logic():
	...
	if is_chasing:
		...
		# Flip sprite biar goblin selalu ngadep player
		anim.flip_h = direction.x < 0
	else:
		velocity.x = 0


# Kena damage dari player
...
func die():
	...
	if damage_area and damage_area.get_node("CollisionShape2D"):
...
func _try_damage_player():
	...
	# Mainin anim attack pas hit ke player
	anim.play("attack")
